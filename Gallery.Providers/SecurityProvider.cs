...
using Gallery.Framework;
using Gallery.ViewModels.RoleAccess;

namespace Gallery.Providers
{
    public class SecurityProvider : BaseProvider
    {
        public SecurityProvider(IGalleryDbContext context) : base(context)
        {
        }
        ...
        {
            UserLogin user = DataContext
                .UserLogins
                .SingleOrDefault(p => p.UserName == userName && p.Password == password && p.IsActive);
            return user != null;
        }
        ...
        //        throw new GalleryException(String.Format(Messages.DuplicateIdentifier, userLogin.UserName));
